...
if( !function_exists( 'travelbiz_enqueue' ) ):
/**
...
* @return void
*/
function travelbiz_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'travelbiz_scripts' , $scripts );
...
if( ! function_exists( 'travelbiz_get_related_posts' ) ):
/**
* Get related posts
*
* @since Travelbiz 1.0.0
...
* 
*/
function travelbiz_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function travelbiz_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'travelbiz_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'travelbiz_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function travelbiz_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'travelbiz' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'travelbiz_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function travelbiz_get_youtube_id( $url ) {
...
if( ! function_exists( 'travelbiz_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Travelbiz 1.0.0
* @uses travelbiz_sort_category()
* @return array
*/
function travelbiz_get_the_category( $id = false ){
    $failed = true;
    ...
    if( $failed ){
      ...
      usort( $cat, 'travelbiz_sort_category' );  
    }
...
if( ! function_exists( 'travelbiz_sort_category' ) ):
/**
* Helper function for travelbiz_get_the_category()
*
* @since Travelbiz 1.0.0
*/
function travelbiz_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'travelbiz_explode_string_to_int' ) ):
/**
...
*/
function travelbiz_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'travelbiz_get_menu' ) ):
/**
* Returns wp nav
*
* @since Travelbiz 1.0.0
...
* @return String
*/
function travelbiz_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'travelbiz_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'travelbiz_'.$location.'_menu', $args ) );
        break;

        case 'footer':
            $args[ 'fallback_cb' ] = 'travelbiz_footer_menu_fb';
...
if( !function_exists( 'travelbiz_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Travelbiz 1.0.0
*/
function travelbiz_hex2rgba($color, $opacity = false) {
