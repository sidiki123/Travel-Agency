...
require_once get_parent_theme_file_path( '/modules/customizer/framework/control-category.php' );
...
if( ! class_exists( 'Travelbiz_Customizer' ) ):

	class Travelbiz_Customizer{
		...
		* 
		* @since  Travelbiz 1.0.0
		* @uses   filter "Travelbiz_Customizer_defaults"
		* @access public
		* @return array
		*/
	    public function get_defaults(){

	    	return apply_filters( 'Travelbiz_Customizer_Defaults', array() );
	    }
		...
		* @since  Travelbiz 1.0.0
		* @uses   get_defaults()
		* @uses   filter "Travelbiz_Customizer_Fields"
		* @access public
		* @param  string $id
		* @return string | false
		*/
		public function get( $id ){
			...
			$fields   = apply_filters( 'Travelbiz_Customizer_Fields', array() );
		...
		*/
		public function register( $wp_customize ){
			...
			do_action( 'travelbiz_customize_register', $wp_customize );

			$this->customize = $wp_customize;

			$this->fields   = apply_filters( 'Travelbiz_Customizer_Fields', array() );
			$this->panels   = apply_filters( 'Travelbiz_Customizer_Panels', $this->panels );
			$this->sections = apply_filters( 'Travelbiz_Customizer_Sections', $this->sections );

			$this->defaults = apply_filters( 'Travelbiz_Customizer_Defaults', $this->defaults );
			...
			foreach( $this->fields as $key => $field ){
				...
				switch ( $field[ 'type' ] ) {
					...
					case 'number':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'travelbiz_sanitize_number';
						}
					...
					case 'checkbox':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'travelbiz_sanitize_checkbox';
						}
					...
					case 'radio':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'travelbiz_sanitize_choice';
						}
					...
					case 'dropdown-pages':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'travelbiz_sanitize_dropdown_pages';
					}
					break;
					
					case 'dropdown-posts':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'travelbiz_sanitize_dropdown_posts';
					}
					break;
					...
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'travelbiz_sanitize_dropdown_categories';
					}
		...
		*/
		protected function add_panels(){
			...
			foreach ( $this->panels as $id => $panel ) {
				...
				$args = array(
				    'title' => empty( $panel[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'travelbiz' ) : $panel[ 'title' ],
				);
		...
		*/
		protected function add_sections(){
			...
			foreach ( $this->sections as $id => $section ){
				...
				$args = array(
					'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'travelbiz' ) : $section[ 'title' ],
				);
...
* 
* @since  Travelbiz 1.0.0
* @uses   Travelbiz_Customizer::get_instance()->get( $id )
* @return string | bool
*/
if( !function_exists( 'travelbiz_get_option' ) ){

	function travelbiz_get_option( $id ){

		return Travelbiz_Customizer::get_instance()->get( $id );
	}

}

/**
* Retrives default value of Travelbiz_Customizer.
* 
* @since  Travelbiz 1.0.0
* @uses   Travelbiz_Customizer::get_instance()->get_defaults()
* @return object
*/
if( !function_exists( 'Travelbiz_Customizer_get_defaults' ) ){

	function Travelbiz_Customizer_get_defaults(){

		return Travelbiz_Customizer::get_instance()->get_defaults();
	}
}

add_action( 'customize_register', array ( Travelbiz_Customizer::get_instance(), 'register' ) );
